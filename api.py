...
async def analyze_image(
    ...
    # Step 1: Segmentation
    t0 = time.time()
    target_mask = models["sam"].segment_target(
        img_rgb, prompt_points=prompt_points, prompt_labels=prompt_labels
    )
    t_seg = time.time() - t0
    ...
    # Step 3: Curvature
    t0 = time.time()
    gaussian_curv = models["curv"].calculate_gaussian_curvature(
        depth_map, mask=target_mask
    )
    critical_vals, critical_coords = models["curv"].find_critical_points(
    ...
    r_pixel = (
        1.0 / np.sqrt(np.abs(highest_stress_raw)) if highest_stress_raw != 0 else 0
    )
